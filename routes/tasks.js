...
router.route("/all").get(async (req, res) => {
  if (req.session.user)
    return res
      .status(200)
      .render("tasks/all", { taskList: await taskData.getAllTasks() });

  return res.status(400).redirect("/");
});

router.route("/public").get(async (req, res) => {
  if (req.session.user) return res.status(200).render("tasks/public", {});

  return res.status(400).redirect("/");
});

router.route("/private").get(async (req, res) => {
  if (req.session.user) return res.status(200).render("tasks/private", {});

  return res.status(400).redirect("/");
});

router.route("/forum").get(async (req, res) => {
  if (req.session.user) return res.status(200).render("tasks/forum", {});
