...
const createFill = async (
  ...
  let changeDur = [];
  inComplete.map((item) => {
    let durObj = date.addMinutes(
      date.addHours(new Date(0), item.duration.durationH),
      item.duration.durationM
    );
    let durNum = Date.parse(
      date.format(durObj, "YYYY-MM-DD HH:mm"),
      "YYYY-MM-DD HH:mm"
    );
...
const deleteSch = async (userId, schName) => {
  ...
  foundtasks = foundtasks.filter((item) => {
    return item.schedule && item.schedule[schName];
  });
  if (foundtasks) {
