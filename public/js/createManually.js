$(function () {
  ...
  function checkSch() {
    ...
    errorlist = [];
    addStyle(
      !checkStr($("#nameInput").val()),
      $("#nameInput"),
      "nameInput must be strings"
    );
    if (checkStr($("#nameInput").val())) {
      addStyle(
        $("#nameInput").val().trim().length < 2 ||
          $("#nameInput").val().trim().length > 25,
        $("#nameInput"),
        "schedule name must be 2-25 characters"
      );
    }

    if (
      !checkStr($("#startDateInput").val()) ||
      !checkStr($("#EndDateInput").val()) ||
      !checkStr($("#StartAMInput").val()) ||
      !checkStr($("#EndAMInput").val()) ||
      !checkStr($("#StartPMInput").val()) ||
      !checkStr($("#EndPMInput").val())
    ) {
  ...
  function fillCalendar(stDt, edDt, tasks, schName) {
    ...
    $.ajax(requestConfig).then(
      function (data) {
        ...
        let full = data.full;
        let calendarDays = data.calendarDays;
        let dayNum = 0;
        ...
        if (fillStart !== 0) {
          ...
          for (let i = 0; i < 7 - fillStart; i++) {
            $("#schTr00").append(
              `<td class='real'><a href='/schedule' id='${dayNum}' class='cell' days='${days}' >${calendarDays[dayNum]}</a></td>`
            );
            days += oneDay;
            dayNum += 1;
          }
        ...
        for (let i = 0; i < full * 7 - fillStart - fillEnd; i++) {
          ...
          while (
            tasks[0] &&
            days + oneDay > tasks[0].schTime &&
            days <= tasks[0].schTime
          ) {
            let real = $("#" + i).closest("td");

            real.append(
              `<br><a href='/tasks/${tasks[0].taskId}'  schtime='${tasks[0].schTime}' id='${tasks[0].taskId}'class='atask' >${tasks[0].taskName}</a>`
            );
  ...
  $("#viewSch").on("click", function (event) {
    ...
    function loadPage(stDt, edDt, tasks, schName) {
      $("#switch").empty();
      ...
      $.ajax({
        type: "GET",
        url: "/schedule/viewSch",
        success: function (data) {
          allSch = data;
          $.each(Object.keys(data), function (i, item) {
            $("#switch").append(
              `<li><a class='schList' href='/schedule' id='${item}'>${item}</a></li>`
            );
          });

          // function changeSchView(){}
          if(!stDt||!edDt||!tasks||!schName){
            let sch01 = data[Object.keys(data)[0]];
            let stDt = sch01["stDt"];
            let edDt = sch01["edDt"];
            let tasks = sch01["tasks"];
            fillCalendar(stDt, edDt, tasks, Object.keys(data)[0],"data",data);
          }else{
            fillCalendar(stDt, edDt, tasks, schName);
          }

          
          // function load Switch(stDt, edDt, tasks, schName){
            $("#switch")
              .find(".schList")
              .on("click", function (event) {
                ...
                let tasks = sch01["tasks"];
                // fillCalendar(stDt, edDt, tasks, schName);
                $("#switch").find(".schList").off("click");
                loadPage(stDt, edDt, tasks, schName);
              });
          // }
          // loadSwitch();
        },
      ...
      deleteSch.on("click", function (event) {
        ...
        confirmDel.on("click", function (event) {
          ...
          //     }
          // })
          $.ajax({ type: "DELETE", url: "/schedule/" + schName }).then(
            function (data) {
              deleteSch.off("click");
              confirmDel.off("click");
              loadPage();
        ...
        $.ajax(requestConfig).then(
          function (data) {
            ...
            if (taskList.length > 0) {
              ...
              $("#dateFooter").append(
                `<p>Time span: from${startDate} to ${endDate}</p>`
              );
              if (fillStart !== 0) {
                schedule.find("tbody").append("<tr id='schTr00'></tr>");
                ...
                for (let i = 0; i < 7 - fillStart; i++) {
                  $("#schTr00").append(
                    `<td class='real'><a href='/schedule' class='cell' id='${index}' days='${days}' hours='${fullHour}'>${calendarDays[index]},hours left:${
                      fullHour / 3600000
                    }:${fullHour % 3600000}</a></td>`
                  );
                  days += oneDay;
                  index +=1;
                }
              ...
              for (i = 0; i < taskList.length; i++) {
                tasks
                  .find("tbody")
                  .append(
                    `<tr><td><a href='/tasks/${taskList[i].id}' dueTime='${taskList[i].dueTime}' duration='${taskList[i].duration.stamp}' id='${taskList[i]._id}'class='atask' name='${taskList[i].taskName}'>${taskList[i].taskName}(${taskList[i].duration.durationH}:${taskList[i].duration.durationM})</a></td></tr>`
                  );
              }
              ...
              function bindAddEvent() {
                ...
                tasks.find(".tasks a").on("click", function (event) {
                  ...
                  let dueTime = Number(atask.attr("duetime"));
                          ...
                          function (responseMessage) {
                            ...
                            cell.html(
                              `${calendarDays[id]},hours left:${Math.floor(
                                Number(dur) / 3600000
                              )}:${Math.floor((Number(dur) % 3600000) / 60000)}`
                            );
                            atask.off("click");
                            trTask.remove();

                            real.append(
                              `<br><a href='/tasks/${taskId}' duration='${duration}' id='${taskId}'class='atask' name='${taskName}' days='${dateMark}'>${taskName}(${Math.floor(
                                Number(duration) / 3600000
                              )}:${Math.floor(
                                (Number(duration) % 3600000) / 60000
                              )})</a>`
                            );
                  ...
                  $("#taskCell").on("click", function (event) {
                    ...
                    $.ajax(requestConfig).then(
                      function (responseMessage) {
                        ...
                        cell.attr("hours", dur);
                        cell.html(
                          `${calendarDays[id]},hours left:${Math.floor(Number(dur) / 3600000)}:${Math.floor(
                        ...
                        $(".tasks tbody").append(
                          `<tr><td><a href='/tasks/${taskId}' duration='${duration}' id='${taskId}'class='atask' name='${taskName}' >${taskName}(${Math.floor(
                            Number(duration) / 3600000
                          )}:${Math.floor(
                            (Number(duration) % 3600000) / 60000
                          )})</a></td></tr>`
            ...
            function (data) {
              ...
              if (taskList.length > 0) {
                ...
                if (fillStart !== 0) {
                  ...
                  for (let i = 0; i < 7 - fillStart; i++) {
                    $("#schTr00").append(
                      `<td class='real'><a href='/schedule' id='${dayNum}' class='cell' days='${days}' hours='${fullHour}'>${calendarDays[dayNum]},hours left:${
                        fullHour / 3600000
                      }:${fullHour % 3600000}</a></td>`
                    );
                    days += oneDay;
                ...
                for (let i = 0; i < durationDays + 1; i++) {
                  ...
                  while (taskList[0] && hours >= taskList[0].duration.stamp) {
                    let real = cell.closest("td");
                    real.append(
                      `<br><a href='/tasks/${taskList[0]._id}' duration='${
                        taskList[0].duration.stamp
                      }' dueTime='${taskList[0].dueTime}' id='${
                        taskList[0]._id
                      }'class='atask'  name = ${taskList[0].taskName}>${
                        taskList[0].taskName
                      }(${Math.floor(
                        Number(taskList[0].duration.stamp) / 3600000
                      )}:${Math.floor(
                        (Number(taskList[0].duration.stamp) % 3600000) / 60000
                      )})</a>`
                    );
                  ...
                  cell.html(
                    `${calendarDays[i]},hours left:${Math.floor(Number(hours) / 3600000)}:${Math.floor(
                      (Number(hours) % 3600000) / 60000
                    )}`
                  );
                }
