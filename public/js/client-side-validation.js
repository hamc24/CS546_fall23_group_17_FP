...
function checkRegister() {
  ...
  addStyle(
    !checkStr($("#firstNameInput").val()),
    $("#firstNameInput"),
    "firstNameInput must be strings"
  );
  if (checkStr($("#firstNameInput").val())) {
    addStyle(
      $("#firstNameInput").val().trim().length < 2 ||
        $("#firstNameInput").val().trim().length > 25 ||
        Array.from($("#firstNameInput").val()).filter(
          (elem) => !elem.match(/[a-zA-Z]/)
        ).length > 0,
      $("#firstNameInput"),
      "firstname must be 2-25 alphabet characters"
    );
  }

  addStyle(
    !checkStr($("#lastNameInput").val()),
    $("#lastNameInput"),
    "lastNameInput must be strings"
  );

  if (checkStr($("#lastNameInput").val())) {
    addStyle(
      $("#lastNameInput").val().trim().length < 2 ||
        $("#lastNameInput").val().trim().length > 25 ||
        Array.from($("#lastNameInput").val()).filter((elem) => {
          elem.match(/[a-zA-Z]/);
        }).length > 0,
      $("#lastNameInput"),
      "lastname must be 2-25 alphabet characters"
    );
  }
  addStyle(
    !checkStr($("#userNameInput").val()),
    $("#userNameInput"),
    "userNameInput must be strings"
  );

  if (checkStr($("#userNameInput").val())) {
    addStyle(
      $("#userNameInput").val().trim().length < 2 ||
        $("#userNameInput").val().trim().length > 25 ||
        Array.from($("#userNameInput").val()).filter((elem) => {
          elem.match(/[a-zA-Z]/);
        }).length > 0,
      $("#userNameInput"),
      "400:userName must be 2-25 alphabet characters"
    );
  }
  ...
  addStyle(
    !checkStr($("#emailAddressInput").val()),
    $("#emailAddressInput"),
    "emailAddress must be strings"
  );

  addStyle(
    !checkStr($("#passwordInput").val()),
    $("#passwordInput"),
    "password must be strings"
  );

  if (checkStr($("#passwordInput").val())) {
    let password = $("#passwordInput").val();
    addStyle(
      Array.from(password).filter((elem) => {
        elem.match(" ");
      }).length > 0
        ? true
        : false,
    ...
    addStyle(
      $("#passwordInput").val().length < 8 || !special || !capital || !numbers,
      $("#passwordInput"),
      "400:invalid password. There needs to be more than 8 characters and contains at least one uppercase character, one number one special characte"
    );
  }

  addStyle(
    !checkStr($("#confirmPasswordInput").val()),
    $("#confirmPasswordInput"),
    "confirm password must be strings"
  );
  ...
  if ($("#confirmPasswordInput").val()) {
    addStyle(
      $("#confirmPasswordInput").val() !== $("#passwordInput").val()
        ...
        : false,
      $("#confirmPasswordInput"),
      "400: confirm password input is not the same as password input"
    );
  }
...
function checkTask() {
  ...
  if (checkStr($("#nameInput").val())) {
    addStyle(
      $("#nameInput").val().trim().length < 2 ||
        $("#nameInput").val().trim().length > 50,
      $("#firstNameInput"),
      "nameInput must be 2-50 characters"
    );
  }
  addStyle(
    !checkStr($("#descriptionInput").val()),
    $("#descriptionInput"),
    "descriptionInput must be strings"
  );
  if (checkStr($("#descriptionInput").val())) {
    addStyle(
      $("#descriptionInput").val().trim().length < 2 ||
        $("#descriptionInput").val().trim().length > 250,
      $("#descriptionInput"),
      "descriptionInput must be 2-250 characters"
    );
  }
  ...
  addStyle(
    ...
    $("#durationInputH"),
    "Task Duration hour cannot be more than 24 hours long and less than 0"
  );
  addStyle(
    !checkNum(dm) || dm < 0 || dm > 60,
    $("#durationInputM"),
    "Task Duration minutes cannot exceed 60 mins or be less than 0"
  );
  addStyle(
    !checkNum(co),
    $("#maxContributorInput"),
    "maxContributorInput must be numbers"
  );
